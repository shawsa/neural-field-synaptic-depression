...
FIG_FILE_NAME = os.path.join(experiment_defaults.media_path, "figure3_alt2")
...
delta_eps = epsilons[1] - epsilons[0]
fd_arr = (responses[2:] - responses[:-2]) / (2*delta_eps)
ax_response.plot(
        epsilons[1:-1], fd_arr, "go", label="Simulation"
)

exact = fd_arr[9]
error = abs(response_slope - exact)/(exact)
print(f"Slope error: {error:.3E}")

ax_response.set_xlabel("$\\varepsilon$")
ax_response.set_ylabel("$\\frac{d}{d\\varepsilon}\\nu_\\infty$")
ax_response.set_xticks([-0.08, 0, 0.08])
ax_response.set_yticks([30, 40, 50, 60, 70, 80, 90, 100])
ax_response.set_ylim([30, 100])
