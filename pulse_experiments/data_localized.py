...
FILE_NAME = 'spatially_localized.pickle'
...
time = TimeDomain_Start_Stop_MaxSpacing(0, 10, 1e-3/5)
...
xis, Us, Qs = pulse_profile(xs_right=xs_right, xs_left=xs_left,
                            c=c, Delta=Delta, **params_dict)
...
base_loc = find_roots(space.inner, u[space.inner_slice]-params_dict['theta'], window=3)[-1]

locations = np.linspace(-10, 10, 101)
responses = []
for loc in tqdm(locations):
    pulse_profile = np.zeros_like(u0)
    pulse_profile[0] += stim_amp * (np.heaviside(space.array-loc+stim_width/2, 0) -
