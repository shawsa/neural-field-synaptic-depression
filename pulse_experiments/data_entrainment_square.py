...
FILE_NAME = os.path.join(experiment_defaults.data_path, "entrainment_square.pickle")
...
for stim_speed, stim_magnitude in tqdm(list(product(stim_speeds, stim_magnitudes))):
    ...
    def stim_func(t):
        return stim_magnitude * (
            np.heaviside(-(space.array - stim_start - stim_speed * t), 0.5)
            - np.heaviside(
                -(space.array - stim_start - stim_speed * t) - stim_width, 0.5
            )
        )
    ...
    for t, (u, q) in zip(time.array, solver.solution_generator(u0, rhs, time)):
        ...
        if len(fronts) < window_width:
            continue
        front_speed = linregress(
            time.spacing * np.arange(window_width), fronts[-window_width:]
        ).slope
...
plt.pcolormesh(mag_mat, stim_mat, res_mat, cmap="seismic", shading="gouraud")
plt.plot(mag_mat, stim_mat, ".", color="gray")
# plt.colorbar(label='$\\nu_\\infty$')
plt.plot(
    stim_magnitudes,
    params_dict["c"] + stim_magnitudes * slope * (1 - np.exp(-stim_width / c)),
    "w-",
    label="Asymptotic",
)
...
with open(FILE_NAME, "wb") as f:
    pickle.dump(
        (stim_magnitudes, stim_speeds, results, stim_width, slope, params, params_dict),
        f,
    )
