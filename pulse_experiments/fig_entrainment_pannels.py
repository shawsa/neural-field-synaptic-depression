...
FILE_NAME = os.path.join(experiment_defaults.media_path, "entrainment_pannel")
...
for pannel, stim_speed in [("success", 1.5), ("failure", 2.0)]:

    def stim_func(t):
        return stim_magnitude * np.exp(
            -np.abs(space.array - stim_start - stim_speed * t) ** 2
        )
