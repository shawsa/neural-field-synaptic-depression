...
FILE_NAME = os.path.join(experiment_defaults.media_path, "entrainment_test.gif")
...
def stim_func(t):
    return stim_magnitude * (
        ...
        - np.heaviside(-(space.array - stim_start - stim_speed * t) - stim_width, 0.5)
    )
    ...
    for index, (t, (u, q)) in enumerate(
        ...
        if len(fronts) > window_width:
            ...
            front_speed = linregress(
                [time.spacing * steps_per_frame] * np.arange(window_width),
                fronts[-window_width:],
            ).slope
