...
plt.pcolormesh(time.array[y_slice], space.array[x_slice], sol_array, cmap="Greys")
plt.ylabel("$x$")
plt.xlabel("$t$")

plt.plot(time.array[y_slice], fronts[y_slice], "b-", label="measured", linewidth=3)
plt.plot(
    time.array[y_slice],
    (
...
for extension in [".png", ".pdf"]:
    plt.savefig(os.path.join(experiment_defaults.media_path, FILE_NAME + extension))
