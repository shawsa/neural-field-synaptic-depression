...
FILE_NAME = os.path.join(
    experiment_defaults.data_path, "entrainment_square_time.pickle"
)
...
def check_entrainment(param_tuple):
    ...
    def stim_func(t):
        return (
            stim_magnitude
            * np.exp(-np.abs(space.array - stim_start - stim_speed * t) ** 2)
            * stim_time_modulation(t)
        )
...
with mp.Pool(7) as pool:
    results = list(
        tqdm(pool.imap(check_entrainment, param_list), total=len(param_list))
    )
...
for ax, mag in zip(axes, stim_magnitudes):
    ...
    for index, (freq, speed) in enumerate(zip(freq_mat.flat, stim_mat.flat)):
        for sol in results:
            if (
                sol["stim_magnitude"] == mag
                and sol["stim_speed"] == speed
                and sol["stim_freq"] == freq
            ):
                np.ravel(res_mat)[index] = int(sol["entrained"])

    temp = ax.pcolormesh(freq_mat, stim_mat, res_mat, cmap="seismic", shading="gouraud")
...
with open(FILE_NAME, "wb") as f:
    pickle.dump(
        (stim_magnitudes, stim_speeds, stim_freqs, results, params, params_dict), f
    )
