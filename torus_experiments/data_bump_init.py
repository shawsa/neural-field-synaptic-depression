...
time = TimeDomain_Start_Stop_MaxSpacing(0, 200, 1e-2)
...
peak_index_x = np.argmax(u[0].ravel()) % space.X.shape[1]
center_index = space.X.shape[1]//2

u_shift = np.roll(u, center_index-peak_index_x, axis=2)

with open(FILE_NAME, "wb") as f:
    pickle.dump((params, space.X, space.Y, u_shift), f)
