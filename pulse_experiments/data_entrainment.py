...
stim_magnitues = np.linspace(0, 0.5, 21)

for stim_speed, stim_magnitude in tqdm(list(product(stim_speeds, stim_magnitues))):
    def stim_func(t):
...
plt.xlim(np.min(stim_magnitues), np.max(stim_magnitues))
plt.ylim(np.min(stim_speeds), np.max(stim_speeds))

mag_mat, stim_mat = np.meshgrid(stim_magnitues, stim_speeds)
...
with open(FILE_NAME, 'wb') as f:
    pickle.dump((stim_magnitude, stim_speeds, results, params, params_dict), f)
